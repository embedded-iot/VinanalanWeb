...
import AddIncomeUtility from "./AddInFurniture";
import ButtonList from "../../../components/commons/ButtonList/ButtonList";
import ViewInFurniture from "./ViewInFurniture";
...
class InFurnitures extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dataSource: [],
        ...
        return (
          <div className="actions-column">
            ...
            <Tooltip title={STRINGS.EDIT}>
              <span className="icon icon-edit" onClick={() =>this.editInFurniture(id)}></span>
            </Tooltip>
            <Tooltip title={STRINGS.ACTION_DELETE}>
              <span className="icon icon-delete" onClick={() =>this.deleteInFurniture(id)}></span>
            </Tooltip>
          </div>
        )
      },
  ...
  onChange = (pagination = {}, filters = {}, sorter  = {}, extra, searchText) => {
    ...
    this.setState({
      ...
      loading: true });
    dispatch(spinActions.showSpin());
    Services.getInFurnitures({...params,}, (response) => {
      dispatch(spinActions.hideSpin());
      const tableSettings = {
        ...this.state.tableSettings,
  ...
  render() {
    ...
    const TableConfig = {
      ...
      tableSettings: tableSettings,
      placeholder: intl.formatMessage({id: 'ENTER_KEYWORD_SEARCH_FOR_FURNITURES'})
    };
    const buttonList = [
...
export default injectIntl(withRouter(connect()(InFurnitures)));