...
const STRINGS = {
  ADD_OUT_FURNITURE: <FormattedMessage id="ADD_OUT_FURNITURE" />,
  EDIT_OUT_FURNITURE: <FormattedMessage id="EDIT_OUT_FURNITURE" />,
...
class AddOutFurniture extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selected: {
  ...
  handleSubmit = () => {
    ...
    if (isEdit) {
      ...
      Services.editOutFurniture(selected.id, data, response => {
      ...
      Services.createOutFurniture(data,response => {
          dispatch(spinActions.hideSpin());
  ...
  render() {
    ...
    return (
      <Modal title={ isEdit ? STRINGS.EDIT_OUT_FURNITURE : STRINGS.ADD_OUT_FURNITURE}
             centered
             width="600px"
             visible={true}
...
export default injectIntl(connect(mapStateToProps)(AddOutFurniture));
