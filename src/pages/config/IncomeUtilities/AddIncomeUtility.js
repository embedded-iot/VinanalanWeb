...
const STRINGS = {
  ADD_INCOME_UTILITY: <FormattedMessage id="ADD_INCOME_UTILITY" />,
  EDIT_INCOME_UTILITY: <FormattedMessage id="EDIT_INCOME_UTILITY" />,
...
class AddIncomeUtility extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selected: {
  ...
  handleSubmit = () => {
    ...
    if (isEdit) {
      ...
      Services.editIncomeUtility(selected.id, data, response => {
      ...
      Services.createIncomeUtility(data,response => {
          dispatch(spinActions.hideSpin());
...
export default injectIntl(connect(mapStateToProps)(AddIncomeUtility));
